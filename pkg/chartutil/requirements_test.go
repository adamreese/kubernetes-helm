...
func TestRequirementsTagsNonValue(t *testing.T) {
	...
	// tags with no effect
	v := &chart.Config{Values: map[string]interface{}{
...
func newConfig(in string) *chart.Config {
	v, err := ReadValues([]byte(in))
	if err != nil {
		panic(err)
	}
	return &chart.Config{Values: v}
}
